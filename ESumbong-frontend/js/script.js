...
if (form) {
  ...
  function generateTrackingID() {
    // Prefix variations
    const prefixes = ["tr", "TR", "Tr"];
    const prefix = prefixes[Math.floor(Math.random() * prefixes.length)];

    // Generate 3-digit number
    const number = Math.floor(Math.random() * 999) + 1;
    const formattedNumber = String(number).padStart(3, "0"); // converts 1 -> 001
  ...
  form.addEventListener("submit", (e)=>{
    ...
    const name = document.getElementById('anonymous').checked ? "Hidden" : document.getElementById('fullname').value;
    ...
    confirmDetails.innerHTML = `
      ...
      Priority: ${document.getElementById('priority').value}<br>
      Address: ${document.getElementById('address').value}<br>
      Date: ${document.getElementById('dateSubmitted').value}
    `;
  ...
  document.getElementById("confirmBtn").onclick = () => {
    ...
    setTimeout(() => {
      ...
      trackCode.textContent = trackingID;

      successModal.classList.remove("hidden");
    }, 2000); // simulate submit time
  };
